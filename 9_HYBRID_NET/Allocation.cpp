							...
							for(int m=0;m<pairs[idx2].nbEdges;m++){
								...
								if(hbP2[tId2] == false){
									if(comp[tId2][i] == 1 && i <= tId && i <= tId2){
										...
										c.size = 3;
										c.isC = 0;
										c.nbDTM = 0.0; c.nbDTM += isDTM[tId] ; c.nbDTM += isDTM[i];  c.nbDTM += isDTM[tId2];
										...
										for(int o=0;o<pairs[idx3].nbEdges;o++){
											...
											if(hbP3[tId3] == false){
												if(comp[tId3][i] == 1 && i <= tId && i <= tId2 && i <= tId3 && tId3 != tId && i != tId2){
													if((comp[tId3][tId2] != 1 || comp[tId][i] != 1 || scores[tId3][tId2] + scores[tId][i] <= scores[tId3][i] + scores[tId][tId2]) &&
														(comp[i][tId3] != 1 || comp[tId2][tId] != 1 || scores[i][tId3] + scores[tId2][tId] <= scores[i][tId] + scores[tId2][tId3]) &&
														(comp[i][tId2] != 1 || comp[tId2][i] != 1 || comp[tId][tId3] != 1 || comp[tId3][tId] != 1 ||
														scores[i][tId2] + scores[tId2][i] + scores[tId][tId3] + scores[tId3][tId] <= scores[i][tId] + scores[tId][tId2]+ scores[tId2][tId3]+ scores[tId3][i])
														){
														Cycle c;
														c.id = cycles.size();
														c.size = 4;
														c.isC = 0;
														c.nbDTM = 0.0; c.nbDTM += isDTM[tId] ; c.nbDTM += isDTM[i];  c.nbDTM += isDTM[tId2]; c.nbDTM += isDTM[tId3];
